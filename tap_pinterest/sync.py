...
def get_advertiser_ids(client, url, owner_user_id):
...
def sync_rest_endpoint(client, catalog, state, url, stream_name, start_date, endpoint_config, parent_id=None):
    ...
    if endpoint_config.get('owner_user_id'):
        path = 'https://api.pinterest.com/ads/v3/advertisers'
        advertiser_ids = endpoint_config['advertiser_ids'] or get_advertiser_ids(client, path, endpoint_config['owner_user_id'])
    else:
        # At least loop once, with no id.
        advertiser_ids = {''}
...
def sync_async_endpoint(client, catalog, state, url, stream_name, start_date, endpoint_config, parent_id=None):
    ...
    bookmark_query_field = endpoint_config.get('bookmark_query_field', 'start_date')
    if bookmark_query_field:
        body[bookmark_query_field] = datetime.strftime(last_datetime, "%Y-%m-%d")
...
def sync(client, config, catalog, state):
    ...
    now = datetime.strftime(utils.now(), "%Y-%m-%d")
